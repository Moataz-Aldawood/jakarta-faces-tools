...
- [x] **Iteration Variable Support (`var="..."`)**: Deep resolution and auto-complete for iteration variables (e.g., `var="u"` in `<ui:repeat>`, `<h:dataTable>`, `<p:dataTable>`) so `#{u.name}` resolves to the element type of the collection!
