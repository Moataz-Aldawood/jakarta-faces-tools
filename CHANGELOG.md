...
- **In-Code Badges via VS Code Inlay Hints**: Renders clean annotation-style badges (`@ViewScoped : `) directly before Expression Language blocks (`@ViewScoped : #{userController.username}`) in `.xhtml` and `.jsf` files, preserving dot-notation continuity. Includes interactive Markdown hover tooltips and can be toggled via `jakartaFacesTools.showInlineBeanScopes`.
